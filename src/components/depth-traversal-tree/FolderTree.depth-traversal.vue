...
const handleSelect = (folderNode: FolderNode) => {
  if (folderNode.folder === selectedFolder.value)
    selectedFolder.value = null
  else
    selectedFolder.value = folderNode.folder
...
const visibleNodes = computed(() => {
  ...
  while (i < folderTreeNodes.value.length) {
    ...
    i++
    while (i < folderTreeNodes.value.length && folderTreeNodes.value[i].depth > parentNodeDepth) {
      i++
    }
  }
  return resultArray
